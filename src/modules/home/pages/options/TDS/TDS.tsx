...
const CustomYearDropdown = ({ data, value, onChange }) => {
  return (
    <FlatList
...
const TDS = () => {
  ...
  const [assessmentYearData, setAssessmentYearData] = useState([]); // Set appropriate type for assessmentYearData
  const [loader, showLoader] = useState(false);
  const [isFocusAssessmentYear, setIsFocusAssessmentYear] = useState(false);
  const [assessmentYearValue, setAssessmentYearValue] = useState('');
  const [certificateList, setCertificateList] = useState([]);

  const BASEURL = 'https://www.vguardrishta.com';

  const handleDropdownFiscalYear = (item) => {
